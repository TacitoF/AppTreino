...
import { apiFetch } from '../auth'; // <--- A CORREÇÃO FOI AQUI (../ em vez de ./)
...
export default function TelaPerfil({ usuario, onSalvar, onVoltar, mostrarToast }) {
  ...
  const [senha, setSenha] = useState(''); // Deixe em branco se não quiser alterar
  ...
  const salvar = async () => {
    ...
    try {
      ...
      await apiFetch('/api/usuario/editar', { method: 'POST', body: payload });
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex flex-col pb-10">
      ...
      <div className="px-5 pt-6 flex flex-col gap-4">
        ...
        <button onClick={salvar} disabled={salvando} className="btn w-full py-5 bg-[#c8f542] active:bg-[#b0d93b] text-black font-black text-base rounded-2xl flex items-center justify-center gap-2 mt-4 shadow-[0_0_20px_rgba(200,245,66,0.1)]">
